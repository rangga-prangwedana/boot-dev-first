...
class Dragon(Unit):
    ...
    def breathe_fire(self, x, y, units):
        x_min, x_max= x - self.__fire_range, x + self.__fire_range
        y_min, y_max= y - self.__fire_range, y + self.__fire_range

        victims = []

        for unit in units:
            if x_min <= unit.pos_x <= x_max and y_min <= unit.pos_y <= y_max:
                victims.append(unit)
        return victims
